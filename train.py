...
def train(config: DictConfig) -> None:
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=config.train_batch_size, shuffle=True, num_workers=config.data.dataloader.num_workers, pin_memory=True)
    ...
    if 'name' not in config.lr_scheduler or config.lr_scheduler.name == 'cosine':
        ...
        lr_scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=len(train_dataloader)*config.num_epochs//config.gradient_accumulation_steps, num_cycles=num_cycles)
