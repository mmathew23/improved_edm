...
class TimestepEmbedding(nn.Module):
    def __init__(
        ...
        self.linear_1 = linear_cls(in_channels, time_embed_dim, bias=BIAS)
        self.non_linearity = nn.SiLU()
        self.linear_2 = linear_cls(time_embed_dim, time_embed_dim, bias=BIAS)

    def forward(self, sample):
        sample = self.linear_2(self.non_linearity(self.linear_1(sample)))
        return sample
...
class Upsample2D(nn.Module):
    ...
    def __init__(
        ...
        conv = None
        if use_conv:
            conv = Conv2d(self.channels, self.out_channels, 3, padding=1, bias=BIAS)
...
class Downsample2D(nn.Module):
    ...
    def __init__(
        ...
        conv = None
        if use_conv:
            conv = Conv2d(self.channels, self.out_channels, 3, stride=stride, padding=padding, bias=BIAS)
        self.conv = conv
...
class ResnetBlock2D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.norm1 = nn.GroupNorm(num_groups=self.groups, num_channels=in_channels, eps=eps, affine=True)
        if self.block_type == 'down':
            self.conv1 = conv_cls(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=BIAS)
        else:
            assert self.block_type == 'up'
            self.conv1 = conv_cls(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=BIAS)

        if temb_channels is not None:
            # *2 due to scale shift
            self.time_emb_proj = linear_cls(temb_channels, out_channels*2, bias=BIAS)

        self.norm2 = nn.GroupNorm(num_groups=self.groups_out, num_channels=out_channels, eps=eps, affine=True)
        self.dropout = torch.nn.Dropout(dropout)
        self.conv2 = conv_cls(out_channels, conv_2d_out_channels, kernel_size=3, stride=1, padding=1, bias=BIAS)
    ...
    def forward(
        ...
        hidden_states = self.norm2(self.conv1(hidden_states))
        ...
        tscale, tshift = torch.chunk(temb, 2, dim=1)
        hidden_states = hidden_states * (1+tscale) + tshift
...
def patch_attention_linear_layers(attention):
    ...
    attention.to_q = Linear(attention.to_q.in_features, attention.to_q.out_features, bias=BIAS)
    attention.to_k = Linear(attention.to_k.in_features, attention.to_k.out_features, bias=BIAS)
    attention.to_v = Linear(attention.to_v.in_features, attention.to_v.out_features, bias=BIAS)
    attention.to_out[0] = Linear(attention.to_out[0].out_features, attention.to_out[0].out_features, bias=BIAS)
...
def recursive_zero_init(module: nn.Module, a: float = -1.0, b: float = 1.0):
    if isinstance(module, UNet2DModel):
        nn.init.zeros_(module.conv_out.weight.data)
        if hasattr(module.conv_out, 'bias'):
            nn.init.zeros_(module.conv_out.bias.data)
    elif isinstance(module, ResnetBlock2D):
        nn.init.zeros_(module.conv2.weight.data)
        if hasattr(module.conv2, 'bias'):
            nn.init.zeros_(module.conv2.bias.data)
    elif isinstance(module, Attention):
        nn.init.zeros_(module.to_out[0].weight.data)
        if hasattr(module.to_out[0], 'bias'):
            nn.init.zeros_(module.to_out[0].bias.data)
    for name, child in module.named_children():
        recursive_zero_init(child, a=a, b=b)
    ...
    for name, child in module.named_children():
        if isinstance(child, (nn.Conv2d, nn.Linear)):
            ...
            if child.bias is not None:
                ...
                nn.init.uniform_(child.bias, a=-scale, b=scale)
        else:
...
class UNet2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.conv_in = Conv2d(in_channels, block_out_channels[0], kernel_size=3, padding=(1, 1), bias=BIAS)
        ...
        # loss weighting 
        self.loss_mlp = nn.Sequential(GaussianFourierProjection(embedding_size=block_out_channels[0], scale=0.25), Linear(timestep_input_dim, 1, bias=BIAS))
        ...
        self.conv_out = Conv2d(block_out_channels[0], out_channels, kernel_size=3, padding=1, bias=BIAS)

        # init weights to normal since weight normalization
        recursive_he_uniform_init(self)
        recursive_zero_init(self)
    ...
    def forward(
        ...
        sample = sample*c_in[:, None, None, None]
        ...
        sample = self.conv_out(self.conv_out_act(self.conv_norm_out(sample))) * c_out[:, None, None, None]
